...
const META_ASSISTANT_QUERIES = new Set([
  "about",
  "about you",
  "who are you",
  "who are you?",
...
const ABOUT_MOHANA_PATTERN = /about (mohana|bhat|the candidate|the person)/;
const PROJECT_QUERY_PATTERN = /\b(project|projects|portfolio|github|live link|demo)\b/;
const ABOUT_MOHANA_INSTRUCTION =
...
const META_ASSISTANT_INSTRUCTION =
...
const OFF_TOPIC_INSTRUCTION = `GUARDRAIL — OFF-TOPIC QUESTION:
...
const PROJECT_FORMAT_INSTRUCTION =
  "For project questions, format the answer as short Markdown bullet lists, not a table. Put resume-highlighted projects first. If you use OTHER GITHUB PROJECTS, place them under a separate **Other Projects** heading. Each item should use this shape: **Project name** — one concise description. Links: [Live](url) | [GitHub](url). Keep links on the same line as their project when possible.";
const CONTEXT_USAGE_INSTRUCTIONS = `Use the latest user message as the primary request. Use the recent conversation only to resolve follow-up wording, references like "that", and what the user is currently asking about. Do not keep answering an earlier question once the user asks a new one.

Use compact Markdown labels like [Live] or [GitHub] with the full URL in the href. Do not print raw full URLs unless the user explicitly asks for them.
For resume projects, use links only from the "RESUME CONTACT AND PROJECT LINKS" context block. If a project has both [Live] and [GitHub], include both. Never invent or reuse a link from another project.
The "OTHER GITHUB PROJECTS" context block is supplemental. Use it for questions about more projects, other projects, GitHub work, or broader portfolio coverage. For those repos, use only the links in that block. Always label those repos as **Other Projects** and never describe them as resume-highlighted projects unless they also appear in the resume context.`;
...
function isMetaAssistantQuery(lowerQuery) {
  return (
    META_ASSISTANT_QUERIES.has(lowerQuery) ||
    lowerQuery.includes("about assistant") ||
    lowerQuery.includes("about agent")
  );
}

function getIntentInstruction(lowerQuery) {
  if (GREETINGS.includes(lowerQuery)) {
    return GREETING_INSTRUCTION;
  }

  if (ABOUT_MOHANA_PATTERN.test(lowerQuery)) {
    return ABOUT_MOHANA_INSTRUCTION;
  }

  if (isMetaAssistantQuery(lowerQuery)) {
    return META_ASSISTANT_INSTRUCTION;
  }

  if (isOffTopic(lowerQuery)) {
    return OFF_TOPIC_INSTRUCTION;
  }

  return "";
}

function getFormatInstructions(lowerQuery) {
  return PROJECT_QUERY_PATTERN.test(lowerQuery)
    ? `\n${PROJECT_FORMAT_INSTRUCTION}`
    : "";
}

function getSystemPrompt(query, persona, context) {
  const lowerQuery = query.toLowerCase().trim();
  const intentInstruction = getIntentInstruction(lowerQuery);

  return `${persona.systemPrompt}${intentInstruction ? `\n\n${intentInstruction}` : ""}

${CONTEXT_USAGE_INSTRUCTIONS}
${getFormatInstructions(lowerQuery)}
